<script lang="ts">
	...
	const canViewRoomList = $derived(
		hasCapability(data.user, capabilities.rooms.list),
	);
...
<svelte:head>
	<title
		>{roomData?.name.replaceAll("-", " ") ?? params.roomName} · Partyroom</title
	>
</svelte:head>
...
{:else if !auth.isAuthenticated || !data.user || !roomId}
	<div
		class="mx-auto flex min-h-[50dvh] w-full max-w-md flex-col items-center justify-center gap-2 p-6 text-center"
	>
		<div
...
{:else}
	...
	>
		<Playback.Root roomId={activeRoomId} {overlayMessages}>
			<header
				class="flex items-center justify-between gap-4 py-[1.1rem] max-[800px]:gap-2 max-[800px]:py-[.7rem]"
			>
				<div class="flex min-w-0 flex-1 items-center gap-3 max-[800px]:gap-1">
					...
					<div class="min-w-0">
						<p
							class="mb-[.2rem] text-[.625rem] font-[650] tracking-[.14em] text-muted-foreground uppercase max-[800px]:text-[.55rem]"
						>
							Listening room <span
								class="inline font-normal tracking-normal normal-case min-[800px]:hidden"
								>· {onlineUsers.length} here</span
							>
						</p>
						<h1
							class="overflow-hidden font-heading text-[1.1rem] font-[650] tracking-[-.025em] text-ellipsis whitespace-nowrap capitalize max-[800px]:text-[.85rem]"
							title={roomData?.name}
						>
							{roomData?.name.replaceAll("-", " ")}
						</h1>
					</div>
				</div>
				<div class="flex min-w-0 items-center gap-3 max-[800px]:gap-1">
					<div
						class="mr-2 flex min-w-0 items-center gap-3 whitespace-nowrap text-xs text-muted-foreground max-[800px]:hidden"
					>
					...
					<Playback.Share>
						{#snippet trigger({ props })}
							<button {...props} class={buttonVariants({ size: "sm" })}
								><Users size={16} /> Invite</button
							>
						{/snippet}
					</Playback.Share>
			...
			<div
				class="grid min-h-0 flex-1 grid-cols-[minmax(0,1fr)_24rem] gap-4 max-[1100px]:grid-cols-[minmax(0,1fr)_21rem] max-[800px]:grid-cols-1 max-[800px]:grid-rows-[auto_minmax(0,1fr)] max-[800px]:gap-3 [@media(height<600px)]:min-h-128"
			>
				<section
				...
				>
					<div
						class="flex items-center justify-between gap-3 px-5 py-4 max-[800px]:px-3 max-[800px]:py-2"
					>
						<span
							class="flex items-center gap-2 text-[.625rem] tracking-[.16em] text-(--stage-muted) max-[800px]:text-[.55rem]"
							><Radio size={15} /> THE STAGE</span
						>
						<div
							class="flex gap-[.4rem] [&_button]:rounded-lg [&_button]:border-(--stage-tool-border) [&_button]:bg-(--stage-tool-background) [&_button]:text-(--stage-tool-foreground) [&_button]:shadow-none [&_button:hover]:bg-(--stage-tool-hover) max-[800px]:[&_button]:h-[1.9rem] max-[800px]:[&_button]:px-2 max-[800px]:[&_button]:text-[.65rem]"
						>
							<Playback.TvMode />
						</div>
					</div>
					<div
						class="flex min-h-0 flex-1 items-center justify-center bg-(--stage-screen) [container-type:size] [&_[data-slot=playback-player]:not(.fixed)]:w-full [&_[data-slot=playback-player]:not(.fixed)]:max-w-[calc(100cqh*16/9)] [&_[data-slot=playback-player]:not(.fixed)]:rounded-none [&_[data-slot=playback-player]:not(.fixed)]:border-0 [&_[data-slot=playback-player]:not(.fixed)]:shadow-none max-[800px]:h-[min(24dvh,13rem)] max-[800px]:flex-none min-[640px]:max-[800px]:h-[min(34dvh,22rem)]"
					>
						<Playback.Player>
							{#snippet empty({ hasQueuedMedia })}
								<div
									class="flex aspect-video flex-col items-center justify-center bg-[radial-gradient(ellipse_at_center,var(--stage-glow),transparent_70%)] p-4 text-center"
								>
									<div class="mb-5 text-(--stage-empty-icon) max-[800px]:mb-2">
										<Headphones
											class="max-[800px]:size-[22px]"
											size={32}
											strokeWidth={1.4}
										/>
									</div>
									<h2
										class="font-heading text-[clamp(1rem,2vw,1.8rem)] font-[550] tracking-[-.04em]"
									>
										{hasQueuedMedia
									...
									<p
										class="mt-2 max-w-80 text-[.8rem] leading-[1.6] text-(--stage-subtle) max-[800px]:max-w-68 max-[800px]:text-[.65rem]"
									>
										{hasQueuedMedia
											? "The next ready song will start automatically."
											: "Every room starts with a song. Add one to the queue."}
									</p>
								</div>
							{/snippet}
						</Playback.Player>
					...
					<div
						class="flex items-center gap-[.875rem] p-5 max-[800px]:gap-[.6rem] max-[800px]:px-3 max-[800px]:py-[.6rem]"
					>
						<div
							class="grid size-11 shrink-0 place-items-center rounded-[.8rem] bg-(--stage-icon-background) text-(--stage-accent) max-[800px]:size-8 max-[800px]:rounded-lg"
						>
							<Music2 size={21} />
						</div>
						<div class="min-w-0">
							<Playback.NowPlaying>
								{#snippet children({ title, hasQueuedMedia })}
									<p
										class="mb-[.2rem] text-[.625rem] font-[650] tracking-[.14em] text-(--stage-subtle) uppercase max-[800px]:text-[.5rem]"
									>
										{title ? "Now playing" : "Up next"}
									</p>
									<h2
										class="overflow-hidden font-heading text-base font-[550] tracking-[-.02em] text-ellipsis whitespace-nowrap max-[800px]:text-[.8rem]"
										title={title ?? undefined}
									>
										{title ??
											(hasQueuedMedia
												? "Preparing the next song…"
												: "Your first song goes here")}
									</h2>
								{/snippet}
							</Playback.NowPlaying>
						</div>
				...
				<RoomTabs.Root bind:value={panelTab} workspace>
					<RoomTabs.List aria-label="Room activity">
						<RoomTabs.Trigger value="queue"
							><ListMusic size={17} /> Queue <Playback.QueueCount
							/></RoomTabs.Trigger
						>
						<RoomTabs.Trigger value="lyrics"
							><Mic2 size={17} /> Lyrics</RoomTabs.Trigger
						>
						<RoomTabs.Trigger value="chat"
							><MessageCircle size={17} /> Chat</RoomTabs.Trigger
						>
					</RoomTabs.List>
					<RoomTabs.Content value="queue"><Playback.Queue /></RoomTabs.Content>
					<RoomTabs.Content value="lyrics"
						><RoomLyrics active={panelTab === "lyrics"} /></RoomTabs.Content
					>
		...
		<Drawer.Root bind:open={settingsOpen}>
			<Drawer.Content class="mx-auto max-w-lg">
				<Drawer.Header>
					...
					<Drawer.Description
						>Choose what visitors can do. Changes save automatically.</Drawer.Description
					>
				</Drawer.Header>
				<div class="overflow-y-auto px-4 pb-4">
					<RoomPermissions
						permissions={roomData.memberPermissions}
						onChange={setMemberPermission}
					/>
				</div>
				<Drawer.Footer
